...
feeder.veg <- feeder.veg |>
  filter(Meter %in% 1:25)

# Give natives a value for functional role
for (i in 1:nrow(feeder.veg)) {
  if (isTRUE(feeder.veg$Invasive[i] == "Native") &
    isTRUE(is.na(feeder.veg$Functional[i]))) {
    feeder.veg$Functional[i] <- "Native"
  }
}

# Give non plants a value for functional role
for (i in 1:nrow(feeder.veg)) {
...
# Give non plants a value for invasive
for (i in 1:nrow(feeder.veg)) {
  if (is.na(feeder.veg$Invasive[i])) {
    feeder.veg$Invasive[i] <- "Not plant"
  }
}
