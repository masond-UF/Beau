...
## Purpose of script: Conducts a binomial model on the herbivory data for 
## the feeder project.
...
mod <- glmer(Browse ~ Feeder * Meter + as_factor(Time) + (1|Plot/Direction),
...
plotResiduals(sim.mod, feeder.veg$Plot, quantreg = T) # ok
plotResiduals(sim.mod, feeder.veg$Time, quantreg = T) # ok 
plotResiduals(sim.mod, feeder.veg$Feeder, quantreg = T) # ok
plotResiduals(sim.mod, feeder.veg$Direction, quantreg = T) # ok

# Overdispersion
testDispersion(sim.mod) # ok

# Good model fit (all values within confidence bands)
library(arm)
...
library(car)
4/length(unique(feeder.veg$Plot)) # 0.16 cook's D threshold
...
4/length(unique(feeder.veg$Direction)) # 0.5 cook's D threshold
...
# Adjust for variance
lme4::VarCorr(mod)
total.sd <- sqrt(0.65917^2 + 0.95889^2) # 1.16
total.sd <- sqrt(sum(as.data.frame(lme4::VarCorr(mod))$vcov)) # 1.16
...
# Trend is not significant but let's take a look
emtrends(mod, pairwise~Feeder, var = 'Meter', sigma = total.sd)
emmip(mod, Feeder~ Meter | Time,cov.reduce = range)

# Create dataframes 
feeder.means <- tibble(Feeder = c('Feeder', 'No feeder'),
								Herbivory = c(0.01139, 0.00483),
								LCL = c(0.00725, 0.00301),
								UCL = c(0.01787, 0.00773))
write.csv(feeder.means, 'Results/Herbivory/feeder-means.csv', row.names = FALSE)

time.means <- tibble(Time = c('Year 1', 'Year 1', 'Year 2', 'Year 2'),
								Feeder = c('Feeder', 'No feeder', 'Feeder', 'No feeder'),
								Herbivory = c(0.02505, 0.01070, 0.00514, 0.00217),
								LCL = c(0.01591, 0.00665, 0.00318, 0.00132),
								UCL = c(0.03924, 0.01715, 0.00830, 0.00358))
write.csv(time.means, 'Results/Herbivory/time-means.csv', row.names = FALSE)
...
descdist(feeder.veg$Browse, discrete = TRUE) # negative binomial
